...
3. **ours realm, Authentication, Policies, WebAuthn Passwordless Policy**: confirm user verification `required` and discoverable credential `preferred`. **Realm settings, Login**: confirm `Enable Passkeys` is on; the login page then shows "Sign in with Passkey" and offers passkeys through the browser's autofill. All of this is in the realm file.
4. **ours realm, Authentication, Required actions**: confirm `Configure OTP`, `Recovery Authentication Codes`, and `Webauthn Register Passwordless` are enabled, and that `Configure OTP` has `Add Recovery Codes` on. **Authentication, Flows, browser**: confirm `Recovery Authentication Code Form` is `Alternative` inside `Browser - Conditional 2FA`. Both are in the realm file and were verified together: after the authenticator is enrolled, Keycloak shows the recovery codes page before letting the user in.
