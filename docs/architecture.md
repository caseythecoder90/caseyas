...
|---|---|---|
| `users` | `keycloakId` (the `sub` claim), `username`, `displayName`, `avatarMediaId`, `prefs { theme, timezone, homeCurrency }`, `createdAt`, `lastSeenAt` | `keycloakId` unique; `username` unique. If a known username arrives with a new subject, which only happens when the realm is recreated, the document is re-linked rather than duplicated, so everything that references the user's id survives. |
...
| Recovery codes | Keycloak | The `Configure OTP` required action carries `add-recovery-codes: true`, and the browser flow's `Browser - Conditional 2FA` subflow has `Recovery Authentication Code Form` as an alternative; both are needed, both are in the realm file, and together they put the recovery-codes page right after authenticator enrollment. Verified locally 2026-09-06. Replaces the hand-written backup codes. |
| Passkeys | Keycloak | Fully supported since 26.4 and switched on per realm with `webAuthnPolicyPasswordlessPasskeysEnabled` (the `Enable Passkeys` switch on the Login tab), which is in the realm file. The login page then offers passkeys through the browser's autofill and a "Sign in with Passkey" button, and the browser flow's `Conditional credential` step skips OTP when a passkey was the primary credential. WebAuthn Passwordless policy: discoverable credential `preferred`, user verification `required`. |
...
In-cluster calls from the api to Keycloak use the public hostname, which hairpins through the Ingress. Verified locally on 2026-09-06 with the production image: with `hostname-backchannel-dynamic=true`, a plain in-cluster request to the discovery document gets the public issuer with local token and JWKS endpoints, but Spring's `issuer-uri` discovery insists the document come from the issuer itself, so pointing it at the service name would fail the issuer check. The hairpin is the simple, correct configuration; the backchannel option stays on so a future manual endpoint override is possible.
...
- `Ingress` for `auth.*` routing only `/realms` and `/resources` to Keycloak. Keycloak env: `KC_HOSTNAME=https://auth.caseylovesyas.com`, `KC_HTTP_ENABLED=true`, `KC_PROXY_HEADERS=xforwarded`, `KC_HOSTNAME_BACKCHANNEL_DYNAMIC=true`, `KC_DB=postgres` plus URL and credentials, `KC_HEALTH_ENABLED=true`. The admin console is reached with `kubectl port-forward` and `KC_HOSTNAME_ADMIN` pointing at the forwarded address. Verified locally with the production image on 2026-09-06: `/admin/` redirects to the forwarded address's console and the console loads, while the realm's issuer stays the public host.
...
- Secrets created by hand, never committed: Keycloak DB credentials, bootstrap admin (first boot only), `ours-web` client secret, Mongo credentials, the `ours-storage` secret (R2 endpoint, bucket, keys, and the `MEDIA_HOST` the frontend's CSP allows), VAPID keypair, the locked-note encryption key, the Claude API key if the paste feature is enabled, GHCR pull secret.
...
| 2 | **Plans and the media foundation** | Items of every kind, ideas with votes and link previews, itinerary drag and drop, checklists, budget, documents. Presigned uploads to R2, Kafka and the worker land here doing photo and PDF thumbnails. Plan spans and booked items show on the calendar tab in a read-only month view. Offline bundle and Today mode. Nightly backups of MongoDB and Keycloak's Postgres to R2 land here too, moved up from milestone 7: the moment real bookings are typed in, losing them has to be impossible. Ends with the seed script creating "Japan 2027", 4 to 19 February, Tokyo, Hakuba, and Kyoto, USD home and JPY local, with the default checklists, and with the real Expedia flight and lodging confirmations entered by hand. |
...
| 7 | **Hardening** | NetworkPolicies, audit log, the Keycloak event-listener extension and login alerts, export job, transactional outbox, a restore drill from a backup (the backups themselves run since milestone 2). |
